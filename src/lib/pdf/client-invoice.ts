...
<body>
  <div class="invoice-header">
    <div>
      ...
      ${config.businessName ? `<p style="font-weight:600;margin-top:4px">${config.businessName}</p>` : ""}
      ${config.businessAddress ? `<p style="color:#737373">${config.businessAddress.replace(/\n/g, "<br>")}</p>` : ""}
    </div>
    <div class="invoice-meta">
      <p style="font-size:16px;font-weight:bold">${config.invoiceNumber}</p>
      <p style="color:#737373">Date: ${new Date().toLocaleDateString()}</p>
      ${config.dueDate ? `<p style="color:#737373">Due: ${new Date(config.dueDate).toLocaleDateString()}</p>` : ""}
    </div>
  </div>
  ...
  <div class="addresses">
    <div class="address-block">
      <h4>Bill To</h4>
      <p style="font-weight:600">${config.clientName}</p>
      ${config.clientAddress ? `<p style="color:#737373">${config.clientAddress.replace(/\n/g, "<br>")}</p>` : ""}
    </div>
  </div>
  ...
  <table>
    ...
    <tfoot>
      <tr>
        ...
        <td style="text-align:right">${grandTotal.toFixed(2)} ${currency}</td>
      </tr>
    </tfoot>
  </table>

  ${config.notes ? `<p style="margin-top:16px;color:#737373">Notes: ${config.notes}</p>` : ""}
